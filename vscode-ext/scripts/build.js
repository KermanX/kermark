...
const languages = [
    ...
    { name: 'kermark', language: 'kermark', identifiers: ['kermark', 'kmk'], source: 'text.html.kermark' }
];
...
const buildGrammar = () => {
    let text = fs.readFileSync(path.join(__dirname, '../', 'syntaxes', 'kermark.tmLanguage.base.json'), "utf8");
    text = text.replace(/\s*"__GEN__"\s*:\s*"\{\{languageIncludes\}\}"/, '\n' + indent(4, fencedCodeBlockIncludes()));
    text = text.replace(/\s*"__GEN__"\s*:\s*"\{\{languageDefinitions\}\}"/, '\n' + fencedCodeBlockDefinitions());
    fs.writeFileSync(path.join(__dirname, '../', 'syntaxes', 'kermark.tmLanguage.json'), text);
};
